...
Peer.prototype._handle = function (msg, next) {
    ...
    var rephex = res && res.reply && res.reply.toString('hex');
    ...
    if (req && has(self._recent.request, reqhex)) {
        self.emit('debug', 'discarding recently seen message %s', reqhex);
        return next();
    }
    if (res && has(self._recent.response, reshex)) {
        self.emit('debug', 'discarding recently seen message %s', reshex);
        return next();
    }
    
    if (req) self._recent.request[reqhex] = Date.now();
    if (res) self._recent.response[reshex] = Date.now();
    ...
    if (res && rephex && has(self._response, rephex)) {
        var r = self._response[rephex];
