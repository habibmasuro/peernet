...
function Peer (db, id, addr, opts) {
    if (!(this instanceof Peer)) return new Peer(db, id, addr, opts);
    Duplex.call(this);
    var self = this;
    if (!opts) opts = {};
...
Peer.prototype._handle = function (msg, next) {
    ...
    if (msg.request && msg.request.close) {
        ...
        randomPeers(self.db, msg.request.node.limit)
        ...
        if (has(self._recent, s.hash)) {
            self.emit('debug', 'discarding recently seen message %s',
                s.hash.toString('hex'));
            return;
        };
        self._recent[s.hash] = Date.now();
        
        var hkey = sha(s.hash).toString('hex');
        ...
        if (!self.address) self.address = msg.request.hello.address.toString();
        self._pushMessage({
            response: {
                seq: msg.request.seq,
                hello: { id: self._id }
