...
Peer.prototype._handle = function (msg, next) {
    ...
    if (msg.request && msg.request.close) {
        ...
        var hkey = msg.request.search.hash.toString('hex')
        ...
        if (msg.request.search.hops < 15) {
            self.emit('debug', 'forwarding search request');
            self.emit('search', msg.request.search, function (res) {
                res.hops ++;
                self._pushMessage({
                    request: {
                        seq: msg.request.seq,
