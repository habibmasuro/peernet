...
var methods = module.exports.methods = [
    'add', 'remove', 'connect', 'disconnect', 'own',
    'known:s', 'log:s', 'subnets:s', 'search:s',
    'join', 'part',
...
RPC.prototype.own = function (cb) {
    return this._pn.own(cb);
};

RPC.prototype.join = function (subnets, cb) {
    this._pn.join(subnets, cb);
};

RPC.prototype.part = function (subnets, cb) {
    this._pn.part(subnets, cb);
};
...
RPC.prototype.subnets = function () {
    var s = this._pn.subnets();
    return readonly(s.pipe(through.obj(function (row, enc, next) {
        this.push(JSON.stringify(row) + '\n');
        next();
    })));
...
RPC.prototype.listen = function (opts, cb) {
    ...
    if (proto === 'ws') {
        var server = servers.wsock(self._pn);
        server.on('error', cb);
